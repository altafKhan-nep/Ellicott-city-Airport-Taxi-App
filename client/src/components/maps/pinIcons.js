// Inline lucide SVG strings for Leaflet divIcon markers (divIcon HTML can't
// hold React components). Uses stroke="currentColor" so pins inherit the CSS
// color of their wrapper (.map-pin span). Paths are from lucide-react v1.31.
const svg = (body, size, strokeWidth = 2) =>
  `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${strokeWidth}" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${body}</svg>`;
